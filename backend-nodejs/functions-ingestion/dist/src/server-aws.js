...
app.get("/api/documents/catalog", async (req, res) => {
    ...
    rows.sort((left, right) => {
        const leftCreated = timestampFromIso(String(cosmosMap.get(left.documentId)?.createdAt ??
            cosmosMap.get(left.documentId)?.updatedAt ??
            ""));
        const rightCreated = timestampFromIso(String(cosmosMap.get(right.documentId)?.createdAt ??
