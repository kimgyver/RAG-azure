...
import { getDocumentStore, getSearchStore, getStorageProvider } from "./providers/index.js";
...
function getFileNameFromBlobName(blobName) {
...
export async function processMessage(body) {
    ...
    try {
        ...
        if (!extracted?.text) {
            ...
            if (message.documentId && message.tenantId) {
                await documentStore.upsert({
                    ...
                    status,
                    contentType,
                    errorMessage: status === "failed" ? "No extractable text found." : undefined
                });
            }
        ...
        const sourceText = sourceTextMaxChars > 0 ? text.slice(0, sourceTextMaxChars) : text;
        ...
        console.log(`[worker-aws] Processed: ${blobName} (${chunks.length} chunks, indexed=${indexed})`);
    }
    catch (error) {
