...
function parseQueueMessage(queueEntry) {
    if (typeof queueEntry === "string") {
        try {
            return JSON.parse(queueEntry);
        }
        catch {
...
async function processQueuedDocumentHandler(queueEntry, context) {
    ...
    try {
        ...
        const extracted = await extractDocumentText(blobName, contentType, download);
        if (!extracted) {
            if (message.documentId && message.tenantId) {
                await upsertDocumentMetadata({
                    documentId: message.documentId,
        ...
        const indexed = await indexChunkDocuments(chunks.map((chunk, i) => ({
