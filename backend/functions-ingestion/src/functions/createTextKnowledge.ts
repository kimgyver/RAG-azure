...
type CreateTextKnowledgeRequest = {
  tenantId?: string;
  title?: string;
  text?: string;
};
...
async function createTextKnowledgeHandler(
  ...
  try {
    const payload = (await request.json()) as CreateTextKnowledgeRequest;
    ...
    const responseBody: CreateTextKnowledgeResponse = {
      documentId,
      tenantId: safeTenantId,
      blobName: safeBlobName,
      fileName,
