...
**기본적으로 생성하지 않음:** **Azure AI Search**, **Cosmos DB**, **Azure OpenAI**. 기존 서비스를 연결하려면 Portal **구성(Configuration)** 또는 `terraform.tfvars`의 `extra_app_settings`를 사용하세요. (설정 이름은 [`local.settings.json.example`](../backend-nodejs/functions-ingestion/local.settings.json.example)과 동일)
...
| `python_api_base_url`                    | Python 백엔드 URL (`VITE_PYTHON_API_BASE_URL`)       |
...
> **`terraform apply`가 `Basic VMs: 0` 또는 `Dynamic VMs: 0` 오류로 실패할 경우:** 먼저 `app_service_plan_sku = "Y1"`(Consumption)인지 확인하세요. 그래도 실패하면 같은 구독 내 기존 Linux 플랜의 리소스 ID를 `terraform.tfvars`의 `existing_linux_service_plan_resource_id`에 입력하거나, 쿼터 증가/다른 구독을 사용하세요. 자세한 내용은 [문제 해결](#문제-해결) 참고.

이 스택은 **리소스 그룹**, **스토리지**(blob + Functions 호스트), **Service Bus**(처리 큐), **Linux Function App**(Node 20, extension ~4), **Application Insights**를 생성합니다. 기본적으로 **새 Linux App Service 플랜**(`Y1`, Consumption)을 만듭니다. `Y1`도 실패하면 **`existing_linux_service_plan_resource_id`**로 기존 Linux 플랜을 재사용하세요. Cosmos DB, AI Search, OpenAI 키는 기본적으로 꺼져 있습니다—`extra_app_settings`나 포털에서 직접 설정하세요.

**기본값:** 현재 기본값은 `app_service_plan_sku = "Y1"`입니다.
...
- **Node.js** (LTS, 예: 20) — `frontend`와 `backend-nodejs/functions-ingestion`에서 `npm run build` 필요
...
Node/Python 백엔드를 모두 쓰려면 프론트 `.env`에 두 URL을 지정하세요 (`terraform output api_base_url`, `terraform output python_api_base_url` 참고):
...
`VITE_UPLOAD_API_BASE_URL`는 기존 호환 변수로 계속 사용할 수 있지만, 런타임 선택 UI를 쓰려면 위의 `VITE_NODE_API_BASE_URL` + `VITE_PYTHON_API_BASE_URL` 조합을 권장합니다.
...
- `/.github/workflows/infra-functions-deploy.yml`
  - 트리거: `infra/**`, `backend-nodejs/functions-ingestion/**` 변경 또는 수동 실행
...
Terraform은 기본적으로 **Cosmos DB**와 **Azure AI Search**를 꺼둡니다 (`COSMOS_DB_ENABLED` / `SEARCH_ENABLED`가 `false`이고 endpoint가 비어 있음). [문서 카탈로그](../backend-nodejs/functions-ingestion/src/functions/listDocumentCatalog.ts) API가 **503**이 아닌 **200**을 반환하려면 최소한 **Search** 또는 **Cosmos**를 켜야 합니다 ([문제 해결: 문서 카탈로그 503](#문서-카탈로그-503) 참고).
...
설정 이름은 [`backend-nodejs/functions-ingestion/local.settings.json.example`](../backend-nodejs/functions-ingestion/local.settings.json.example)과 동일합니다.
...
- [ ] **Functions 빌드 포함:** 매번 배포 전 `backend-nodejs/functions-ingestion`에서 `npm run build` 실행. 이 저장소는 `.gitignore`에서 `backend-nodejs/functions-ingestion/dist/`를 제외시켜 Core Tools가 빌드된 JS를 포함하도록 함([404 문제 해결](#function-라우트-404) 참고).
...
- [ ] **API URL:** `frontend/.env`의 `VITE_NODE_API_BASE_URL`이 `terraform output api_base_url`과 일치하고, `VITE_PYTHON_API_BASE_URL`이 `terraform output python_api_base_url`과 일치(둘 다 끝에 `/api` 포함).
...
`func azure functionapp publish`는 **.gitignore**를 따릅니다. 만약 `backend-nodejs/functions-ingestion/dist/`가 무시되고 있었다면, zip에 빌드된 JS가 포함되지 않아 Azure에 라우트가 등록되지 않아 404가 발생합니다. 이 저장소는 `!backend-nodejs/functions-ingestion/dist/`로 예외 처리되어 있습니다. 변경사항을 pull한 뒤 `npm run build` 실행 후 **`func azure functionapp publish …` 재실행**하세요.
...
`frontend/.env`의 **`VITE_NODE_API_BASE_URL=https://<app>.azurewebsites.net/api`**(끝에 `/api` 포함)도 확인하세요. Python 백엔드를 병행한다면 `VITE_PYTHON_API_BASE_URL`도 함께 확인하세요.
...
일부 구독(예: Azure for Students, 제한이 심한 테넌트)은 `Y1`(Dynamic)이나 Basic 쿼터가 0일 수 있습니다. 이 경우 Terraform은 새 App Service 플랜을 만들 수 없습니다.

해결 방법:

1. `terraform.tfvars`에서 `app_service_plan_sku = "Y1"`인지 확인
2. **같은 구독 내 기존 Linux App Service 플랜 재사용:**
   ```hcl
